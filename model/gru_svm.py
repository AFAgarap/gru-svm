...
def main():
	...
	with tf.name_scope('input'):
		...
		# y = tf.one_hot(y_input, 2, dtype=tf.float32, name='y_onehot')
	...
	train_step = tf.train.GradientDescentOptimizer(learning_rate=1e-6).minimize(loss)
	...
	with tf.Session() as sess:
		...
		try:
			...
			# while not coord.should_stop():
				...
				summary, _, train_loss_, outH, accuracy_, y_input_ = sess.run([merged, train_step, loss, H, accuracy, y_input],
					feed_dict = feed_dict)
				if index % 10 == 0:
